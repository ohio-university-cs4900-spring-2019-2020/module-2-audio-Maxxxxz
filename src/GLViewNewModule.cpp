...
void GLViewNewModule::onCreate()
{
   ...
   this->se = irrklang::createIrrKlangDevice();
   this->se->setRolloffFactor(1);
	Aftr::Vector ld = cam->getLookDirection();
	Aftr::Vector pos = cam->getPosition();
   this->se->setListenerPosition(
...
void GLViewNewModule::onKeyDown( const SDL_KeyboardEvent& key )
{
   ...
   {
	   printf("played 3D sound\n");
	   playSound();
   }

   if (key.keysym.sym == SDLK_3)
   {
	   printf("played 3d sound at random location with new waypoint\n");
...
void Aftr::GLViewNewModule::loadMap()
{
   ...
   //skyBoxImageNames.push_back(ManagerEnvironmentConfiguration::getSMM() + "/../photoshop_files/fellows/IMG_2382.jpg");
   ...
   Vector inVector[9] = { Vector(50, 45, 50), Vector(50, 55, 50),		//eyes
						  Vector(50, 50, 45),							//nose
						  Vector(50, 35, 40), Vector(50, 45, 35),		//mouth right
						  Vector(50, 55, 35), Vector(50, 65, 40)		//mouth left
													};

   Vector vec;

   for each (vec in inVector)
   {
	   createNewModuleWayPoints(vec);
   }

}

void GLViewNewModule::playSound()
{
	irrklang::vec3df loc = irrklang::vec3df(0.0f, 0.0f, 10.0f);
	this->se->play3D((ManagerEnvironmentConfiguration::getSMM() + "/sounds/space.ogg").c_str(),
		loc, true);
	createNewModuleWayPoints({0.0f, 0.0f, 10.0f});
}

void GLViewNewModule::createRandomWayPoint()
{
	srand(time(NULL));
	float rx = (rand() % 500) - 250;
	float ry = (rand() % 500) - 250;
	float rz = (rand() % 500) - 250;

	createNewModuleWayPoints({ rx, ry, rz });
	this->se->play3D((ManagerEnvironmentConfiguration::getSMM() + "/sounds/sound5.wav").c_str(),
		{ rx, ry, rz }, false);

}

void GLViewNewModule::createNewModuleWayPoints(Vector in)
{
